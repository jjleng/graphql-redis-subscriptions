...
var RedisPubSub = (function () {
    ...
    RedisPubSub.prototype.subscribe = function (trigger, onMessage, options) {
        ...
        if (refs && refs.length > 0) {
            this.subsRefsMap[triggerName] = __spreadArrays(refs, [id]);
            return Promise.resolve(id);
        }
        else {
            this.subsRefsMap[triggerName] = __spreadArrays((this.subsRefsMap[triggerName] || []), [
                id,
            ]);
            return new Promise(function (resolve, reject) {
                ...
                subscribeFn.call(_this.redisSubscriber, triggerName, function (err) {
                    if (err) {
                        _this.subsRefsMap[triggerName].pop();
                        reject(err);
                    }
                    else {
                        resolve(id);
    ...
    RedisPubSub.prototype.unsubscribe = function (subId) {
        ...
        if (refs.length === 1) {
            ...
            this.subsRefsMap[triggerName] = index === -1
                ? refs
                : __spreadArrays(refs.slice(0, index), refs.slice(index + 1));
        }
